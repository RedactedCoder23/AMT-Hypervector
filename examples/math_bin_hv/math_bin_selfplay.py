...
def main():
    ...
    mu_pos = np.zeros(args.dim); mu_neg = np.zeros(args.dim)
    n_pos = n_neg = 0
    ...
    with open(args.log,"w",newline="") as f:
        ...
        for i in trange(1,args.cycles+1):
            ...
            sample,truth = (random_math_sample() if domain=="math" else random_bin_sample())
            vec = hv.encode(sample,args.dim)
            if truth:
                n_pos+=1; mu_pos=incremental_mean(mu_pos,vec,n_pos); label=1
            else:
                n_neg+=1; mu_neg=incremental_mean(mu_neg,vec,n_neg); label=-1
            score = hv.cosine(vec,mu_pos) - hv.cosine(vec,mu_neg)
            w.writerow({"step":i,"domain":domain,"sample":sample,"label":label,"score":score})
